...
const CreatePost = () => {
    ...
    if (form.prompt) {
      try {
        ...
        const response = await fetch(
          'https://openai-dalle-c588.onrender.com//api/v1/dalle',
          {
            method: 'POST',
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch(
          'https://openai-dalle-c588.onrender.com//api/v1/post',
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
      ...
      alert('Please enter a prompt and generate');
    }
  };
    ...
    <section className='mx-auto max-w-7xl'>
      ...
      <form action='' className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-5'>
          ...
          <div className='justfy-center relative flex h-64 w-64 items-center rounded-lg border border-gray-300 bg-gray-50 p-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500'>
            {form.photo ? (
              <img
                src={form.photo}
                alt={form.prompt}
